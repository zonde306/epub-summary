...
最新实现已在 merged preview 校验通过后、进入审阅队列前接入结构丢失检测。该机制比较当前正式文档与本批次 merged preview 的 YAML 映射路径，仅检查字段路径缺失，不比较字段值变化。

### 检查规则

- 基于键路径集合做差集比较
- 仅递归展开映射节点
- 列表字段本身参与路径比较，但列表内部不按索引展开
- 使用 `previous_paths - current_paths` 判定缺失路径
- 若差集非空，则批次进入结构缺失待审批状态

### 工作流接入点

当前工作流新增 `detect_structure_loss` 节点，链路变为：

1. `merge_delta_preview`
2. `validate_merged_preview`
3. `detect_structure_loss`
4. `enqueue_review`

其中：

- 若无缺失路径，批次按普通审阅进入队列
- 若存在缺失路径，批次仍进入统一审阅队列，但会附带结构缺失元数据，并将恢复动作切换为 `review_structure_loss`
...
其中 `structure_check.json` 会记录：

- 检查时间
- 基准文档与 preview 文件位置
- `actors_missing_paths`
- `worldinfo_missing_paths`
- `missing_paths_count`
- `structure_check_passed`
- `requires_loss_approval`

### 状态与队列扩展

`RunState` 已扩展：

- `pending_loss_review_batch_id`
- `last_structure_check_batch_id`
- `last_structure_check_passed`
- `recommended_action=review_structure_loss`
...
- `worldinfo_missing_paths`
- `requires_loss_approval`
- `loss_approval_status`
- `loss_approval_comment`

`ReviewQueueStore` 已扩展 `review_kind`，用于区分：

- `normal_review`
- `structure_loss_review`
...
当 `run_to_completion()` 遇到结构缺失批次时，不再自动提交，而是立即中止并要求人工审阅，避免在无人确认情况下放行缺失结果。

### CLI 审阅行为

`review-batch` 仍保持单入口，但在结构缺失场景下会额外输出：

- 缺失路径总数
- `actors` / `worldinfo` 缺失数量
- 高优先级告警文案
- 相关文件路径
- 前若干条缺失路径摘要
...
- `accept`：人工确认允许当前缺失结果继续提交
- `reject`：拒绝当前结果并进入重试路径
- `edit`：允许用户先修改 preview，再重新执行结构检查并据结果提交
